          ...
          <v-list-item two-line>
            <v-list-item-content>
              ...
              <v-list-item-subtitle>
                Fast iteration and continuous delivery are key measurements of DevOps success.
              </v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
        </v-card>
            ...
            <template v-slot:default>
              ...
              <tbody>
                <tr class="prodRow">
                  ...
                  <td
                    :class="
                      month.deployments.prodAvgDay >= 0.1
                        ? 'good'
                        : month.deployments.prodAvgDay > 0
                        ? 'ok'
                        : 'bad'
                    "
                  >
                  ...
                  <td
                    :class="
                      month.deployments.prodSuccRate > 90
                        ? 'good'
                        : month.deployments.prodSuccRate > 80
                        ? 'ok'
                        : 'bad'
                    "
                  >
                ...
                <tr class="nonProdRow">
                  ...
                  <td
                    :class="
                      month.deployments.stagAvgDay >= 0.2
                        ? 'good'
                        : month.deployments.stagAvgDay > 0
                        ? 'ok'
                        : 'bad'
                    "
                  >
                  ...
                  <td
                    :class="
                      month.deployments.stagSuccRate > 80
                        ? 'good'
                        : month.deployments.stagSuccRate > 70
                        ? 'ok'
                        : 'bad'
                    "
                  >
                ...
                <tr class="nonProdRow">
                  ...
                  <td
                    :class="
                      month.deployments.featAvgDay >= 0.4
                        ? 'good'
                        : month.deployments.featAvgDay > 0
                        ? 'ok'
                        : 'bad'
                    "
                  >
                  ...
                  <td
                    :class="
                      month.deployments.featSuccRate > 70
                        ? 'good'
                        : month.deployments.featSuccRate > 60
                        ? 'ok'
                        : 'bad'
                    "
                  >
...
export default {
  ...
  created() {
    if (this.$store.state.deployments.deployments.length < 1) {
      this.$store.dispatch('deployments/getDeployments', this.id);
    }
  },
  methods: {
    ...
    deployStatsByTimeWindow(start, end) {
      ...
      const monthlyStats = {
        ...
        prodSuccess: inData.filter(
          (item) => item.environment === 'production' && item.status === 'success'
        ).length,
        stagSuccess: inData.filter(
          (item) => item.environment === 'staging' && item.status === 'success'
        ).length,
        featSuccess: inData.filter(
          (item) => item.environment === 'feature' && item.status === 'success'
        ).length,
        get prodAvgDay() {
          return (this.prodDeploys / this.daysInMonth).toFixed(2);
        },
        get stagAvgDay() {
          return (this.stagDeploys / this.daysInMonth).toFixed(2);
        },
        get featAvgDay() {
          return (this.featDeploys / this.daysInMonth).toFixed(2);
        },
        get prodSuccRate() {
          return ((this.prodSuccess / this.prodDeploys) * 100).toFixed(2);
        },
        get stagSuccRate() {
          return ((this.stagSuccess / this.stagDeploys) * 100).toFixed(2);
        },
        get featSuccRate() {
          return ((this.featSuccess / this.featDeploys) * 100).toFixed(2);
        },
            ...
            inData
              .filter(
                (item) =>
                  ...
                  item.status === 'success' &&
                  item.pipe_duration < 3600000
              )
              .reduce(pipeReducer, pipeTime) /
            1000 /
            this.prodSuccess
          ).toFixed(2);
        },
